-- Pure Lua 5.4 version (no FFI)

-- Constants and Defaults
local PROGRAM = "pdfcrop"
local VERSION = "1.43 (Lua 5.4)"
...
-- Numeric limits
local FALLBACK_PID_MOD = 100000      -- Modulo for fallback PID generation
local TMP_TIME_MOD = 1000000         -- Modulo for temp file uniqueness
local MAX_RESOLUTION = 9999          -- Maximum Ghostscript resolution
local MAX_MARGIN = 10000             -- Maximum margin value (bp) - about 14 feet
local MAX_BBOX_VALUE = 100000        -- Maximum bounding box coordinate value

-- OS Detection (pure Lua) - cached for performance
local IS_WINDOWS = package.config:sub(1, 1) == "\\"
local OS_NAME = IS_WINDOWS and "Windows" or "Unix"

-- Pattern to detect unsafe characters in filenames (requires temp copy)
local UNSAFE_FILENAME_PATTERN = IS_WINDOWS and "[^%w%.%-%_/\\:]" or "[^%w%.%-%_/\\]"
...
-- Get process ID (pure Lua via shell)
local function get_pid()
    if IS_WINDOWS then
        -- Use PowerShell to get PID, or fall back to time-based unique ID
        local handle = io.popen("powershell -NoProfile -Command \"$PID\" 2>nul")
        if handle then
        ...
        -- Fallback: use a combination of time values
        return os.time() % FALLBACK_PID_MOD
    else
        -- Try /proc/self first (more reliable than echo $$ which returns subshell PID)
        local f = io.open("/proc/self/stat", "r")
        if f then
            local stat = f:read("*l")
            f:close()
            if stat then
                local pid = stat:match("^(%d+)")
...
local unlink_files = {}
local tmp_prefix = "tmp-" .. PROGRAM .. "-" .. get_pid() .. "-" .. (os.time() % TMP_TIME_MOD)
...
-- Parse margin string and return left, top, right, bottom values
local function parse_margins(margin_str)
    local m1, m2, m3, m4 = margin_str:match("^%s*([%-%d%.]+)%s+([%-%d%.]+)%s+([%-%d%.]+)%s+([%-%d%.]+)%s*$")
    if m1 then
        return tonumber(m1), tonumber(m2), tonumber(m3), tonumber(m4)
    end

    m1, m2 = margin_str:match("^%s*([%-%d%.]+)%s+([%-%d%.]+)%s*$")
    if m1 then
        return tonumber(m1), tonumber(m2), tonumber(m1), tonumber(m2)
    end

    m1 = margin_str:match("^%s*([%-%d%.]+)%s*$")
    if m1 then
        local v = tonumber(m1)
        return v, v, v, v
    end
...
-- Validate a bounding box value (number, not NaN/Inf, within limits)
local function validate_bbox_value(v)
    return type(v) == "number" and v == v and v ~= math.huge and v ~= -math.huge
           and math.abs(v) < MAX_BBOX_VALUE
end

-- Check if a command is executable (pure Lua)
local function check_executable(cmd)
    if IS_WINDOWS then
        -- Use 'where' command to find executable
        ...
        -- Unix: use 'command -v' or 'which'
        if cmd:find("/") then
            -- Absolute or relative path - check if file exists and is executable
            local f = io.open(cmd, "r")
            if f then
                f:close()
                -- Check if executable using test -x
                local ret = os.execute('test -x "' .. cmd .. '"')
...
-- Search Windows Registry for Ghostscript using reg query (pure Lua)
local function search_registry_hive(hive_name)
    if not IS_WINDOWS then return nil end

    -- Query for Ghostscript subkeys
    local handle = io.popen('reg query "' .. hive_name .. '\\SOFTWARE\\Ghostscript" 2>nul')
    if not handle then return nil end

    local versions = {}
    for line in handle:lines() do
        -- Lines look like: HKEY_LOCAL_MACHINE\SOFTWARE\Ghostscript\10.02.1
    ...
    -- Try each version to find GS_DLL
    for _, ver in ipairs(versions) do
        ...
        if qhandle then
            for line in qhandle:lines() do
                -- Line looks like: GS_DLL    REG_SZ    C:\Program Files\gs\gs10.02.1\bin\gsdll64.dll
                local dll_path = line:match("GS_DLL%s+REG_SZ%s+(.+)$")
                if dll_path then
                    dll_path = dll_path:gsub("%s+$", "") -- trim trailing whitespace
...
-- Argument Parser option tables
local FLAG_OPTIONS = {
    version = {"version", true},
    debug = {"debug", true, "verbose", true},  -- debug also sets verbose
    nodebug = {"debug", false},
    ...
    uncompress = {"uncompress", true},
    nouncompress = {"uncompress", false},
    pdftex = {"tex", "pdftex"},
    xetex = {"tex", "xetex"},
    luatex = {"tex", "luatex"},
}
...
local VALUE_OPTIONS = {
    gscmd = {"gscmd", "gscmd_custom"},
    pdftexcmd = {"pdftexcmd", "pdftexcmd_custom"},
    xetexcmd = {"xetexcmd", "xetexcmd_custom"},
    luatexcmd = {"luatexcmd", "luatexcmd_custom"},
    margins = {"margins"},
    papersize = {"papersize"},
...
local function parse_args(args)
    ...
    while i <= #args do
        ...
        elseif not end_of_options and arg:sub(1, 2) == "--" then
            ...
            if opt == "help" then
                usage(false)
            elseif FLAG_OPTIONS[opt] then
                local flag = FLAG_OPTIONS[opt]
                for j = 1, #flag, 2 do
                    options[flag[j]] = flag[j + 1]
                end
            elseif VALUE_OPTIONS[opt] then
                i = i + 1
                if not args[i] then
                    print("Error: Option --" .. opt .. " requires a value.")
                    usage(true)
                end
                local val_opt = VALUE_OPTIONS[opt]
                options[val_opt[1]] = args[i]
                if val_opt[2] then options[val_opt[2]] = true end
            else
...
local function main()
    ...
    if input_file ~= "-" then
        ...
        f:close()
        if not header or not header:match("^%%PDF%-") then
            print("Error: Input file is not a valid PDF file: " .. input_file)
            cleanup(1)
        end
    end
    ...
    local margin_left, margin_top, margin_right, margin_bottom = parse_margins(options.margins)
    ...
    if input_file == "-" then
        ...
        out:write(content)
        out:close()
    elseif input_file:match(UNSAFE_FILENAME_PATTERN) then
    ...
    if options.restricted then
        if options.gscmd_custom or options.pdftexcmd_custom or
           options.xetexcmd_custom or options.luatexcmd_custom then
...
if options.pdfversion and options.pdfversion ~= "auto" and options.pdfversion ~= "none" then
    ...
    if options.resolution then
        ...
        if not res_num or res_num < 1 or res_num > MAX_RESOLUTION then
            print("Error: Invalid resolution: " .. options.resolution ..
                  " (must be 1-" .. MAX_RESOLUTION .. ")")
            cleanup(1)
        end
    ...
    for line in gs_out_f:lines() do
        ...
        if x1 then
            ...
            if not (validate_bbox_value(box_gs[1]) and validate_bbox_value(box_gs[2]) and
                    validate_bbox_value(box_gs[3]) and validate_bbox_value(box_gs[4])) then
                print(string.format(
                    "!!! Error: Invalid BoundingBox values returned by Ghostscript for page %d!",
                    page))
                gs_out_f:close()
                tmp_f:close()
                cleanup(1)
            ...
            if box[1] >= box[3] or box[2] >= box[4] then
                print(string.format("\n!!! Warning: Empty Bounding Box detected!\n" ..
                    ...
                    page, box_gs[1], box_gs[2], box_gs[3], box_gs[4],
                    box[1], box[2], box[3], box[4]))
                tmp_f:write(string.format("\\pageinclude{%d}\n", page))
            else
                if options.verbose then
                    print(string.format("* Page %d: %g %g %g %g",
                        page, box[1], box[2], box[3], box[4]))
                end

                local bb = {
                    box[1] - margin_left, box[2] - margin_bottom,
                    box[3] + margin_right, box[4] + margin_top
                }
                if bb[1] >= bb[3] or bb[2] >= bb[4] then
                    ...
                    if options.clip then
                        tmp_f:write(string.format(
                            "\\pageclip %d [%g %g %g %g][%g %g %g %g]\n",
                            page, box[1], box[2], box[3], box[4],
